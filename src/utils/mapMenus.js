...
export function mapToMenuRoutes(menuList) {
  // 获取所有路由
  const allRoutes = []
  const routeFiles = require.context("../router/main", true, /\.js$/)//读取路由文件
...
export function pathMapToMenu(userMenus,currentPath,breadcrumbs) {
  for (const menu of userMenus) {
    if (menu.type === 1) {
      ...
      if (findMenu) {
        breadcrumbs?.push({ name: menu.label, id: menu.key })
        breadcrumbs?.push({ name: findMenu.label, id: findMenu.key })
        return findMenu
      }
